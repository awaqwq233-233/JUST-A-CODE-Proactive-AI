...
- **CLI 开关**：`--mic <id>` 指定麦克风、`--no-voicebox` 关克隆声纹桥接（改系统 TTS/仅文本）、`--no-play` 关 omni 自带音频播放（排查用）、`--list-mics` / `--mic-gain` 设备排查、`--no-auto-launch` 不自动拉起 server、`--video-interval <秒>` 图像上行间隔（默认 1.0，0=每段都带图）、**`--no-echo-gate` 强制关闭回声门控（默认按输入+输出设备自动判定：耳机+独立麦→关可打断，输入本身是耳机→开防自激）**。
