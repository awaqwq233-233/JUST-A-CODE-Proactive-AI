...
class VisionDetector:
    ...
    def __init__(self, mode='yolo', yolo_model_path='yolov8n.pt', la_model_path=None):
        """
        初始化检测器
        ...
        if mode in ['locate_anything', 'hybrid']:
            self._init_locate_anything(la_model_path)
    ...
    def _init_locate_anything(self, model_path):
        """初始化 LocateAnything-3B 模型"""
        try:
            import torch
            from transformers import AutoModel, AutoTokenizer, AutoProcessor
            
            model_name = model_path or "nvidia/LocateAnything-3B"
            print(f"[系统] 正在加载 LocateAnything-3B 模型: {model_name} ...")
